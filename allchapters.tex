...
Save this code in a file named \uri{hello.psgi} and then use the
...
\uri{http://127.0.0.1:5000/} and you should see the ``Hello World'' page.
...
This app would serve \uri{favicon.ico} if the request path looks like
\uri{/favicon.ico}, the ``Hello World'' page for requests to the root (\uri{/}) and
...
in a \uri{.psgi} file with one of the PSGI web server backends using Plack
handlers. The usage is simple: just pass a \uri{.psgi} file path to the
command:
...
named \uri{app.psgi} in the current directory.
...
\item
  ...
  \module{AnyEvent}, \module{Coro} or \module{POE} the equivalent and most appropriate backend is
  chosen automatically.
\item
  Otherwise, fallback to the default ``Standalone'' backend implemented
  in the \module{HTTP::Server::PSGI} module.
\end{itemize}
...
\uri{.psgi} or \uri{.pm} files. Because the Plack server
...
\uri{/foo/bar.html} into a local file like
\uri{/path/to/htdocs/foo/bar.html}, opens the file handle, and
...
Using these applications is easy. Just write a \uri{.psgi} file like this:
...
Now you can access any file under your \uri{~/public_html} with
the URL \uri{http://localhost:5000/somefile.html}

You can also use \module{Plack::App::Directory}. This time let's run it with just
the plackup command without a \uri{.psgi} file:
...
\module{Plack::App::*} applications without ever touching a \uri{.psgi} file!
...
a \uri{.psgi} file that looks like this:
...
toolchain of plackup, \module{Plack::Test}, and middleware which we'll discuss in
...
And that will mount the path \uri{/path/to/cgi-bin}, so suppose
you have \uri{foo.pl} in that directory, you can access
\uri{http://localhost:5000/foo.pl} to run the CGI application as a PSGI over
...
we saw how to enable Plack middleware components in \uri{.psgi} file, using
...
statement as the final statement in \uri{.psgi} file as well.
...
Requests to \uri{/john} is handled exactly the same way with the normal
URLMap. But this example uses \lstinline!builder! for \uri{/bob}, so it
...
\uri{.psgi} file), and call \lstinline!test_psgi! function to test
...
Save that code as \uri{.t} file and use the tool such as
...
default to \uri{http://localhost/} anyway.
...
\uri{.cgi} script where you could factor out the application code
into a module (\uri{.pm} files). The same thing applies to
\uri{.psgi} file: it's best to create an application class by
using and possibly extending \module{Plack::Request}, and then have just a few
lines of code in \uri{.psgi} file with
...
This way you run the same \lstinline!$app! in \uri{/public} and
\uri{/private} paths, while \uri{/private} requires a basic authentication
and \uri{/public} doesn't. (Inlining \lstinline!$env->{REMOTE_USER}!, or
whatever application logic in \uri{.psgi} is not really recommended -- i just
...
\uri{/whatever.json} continues to return the JSON but requests to
\uri{/whatever.json?callback=myCallback} would return:
...
This will intercept all requests beginning with \uri{/static} and map that
to the root directory \uri{htdocs}. So requests to
\uri{/static/images/foo.jpg} will result in serving a file
\uri{./htdocs/static/images/foo.jpg}.

Often you want to overlap or cofigure the directory names, so a request
to the URL \uri{/static/index.css} mapped to \uri{./static-files/index.css},
...
\uri{/static/} and in that case, strip off that part, and map the files
under \uri{./static-files/}.

As a result, \uri{/static/foo.jpg} would become
\uri{./static-files/foo.jpg}. All requests not matching the pattern match
...
the URL \uri{http://yourhost.reversehttp.net/} from anywhere in the world.
...
\uri{.psgi}. If it is one-time middleware that you can quickly
...
Do not write your application logic in \uri{.psgi} files using
\module{Plack::Request}. It's like writing a 1000 lines of CGI script using
